...
# class gcn_model(T5ForConditionalGeneration):
#     def __init__(self,config):
#         super().__init__(config)
#         self.conv1 = GCNLayer(config.d_model, config.d_model)
#         # self.conv2 = GCNLayer(config.d_model, config.d_model)
#
#
#     def get_encoder(self):
#         return encoder(self.encoder,self.shared,self.conv1)
#
#
#
...
#             inputs_embeds=self.conv1(inputs_embeds,edges)
#             input_ids=None
#         #############
#
...
#         ######################
#         # hidden_states=self.conv1(hidden_states,edges)
#         # hidden_states=self.conv2(hidden_states,edges)
...
class gcn_model(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    def get_encoder(self):
        return encoder(self.t5model.encoder,self.t5model.shared,self.conv)
    ...
    def forward(self,
        ...
        assert edges != None, '邻接矩阵未输入'
        if (input_ids != None):
            inputs_embeds=self.t5model.shared(input_ids)
            inputs_embeds=self.conv(inputs_embeds,edges)
            input_ids=None
        return self.t5model(input_ids=input_ids,
                            attention_mask=attention_mask,
