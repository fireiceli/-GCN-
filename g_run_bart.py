...
from seq2seq.constrained_bart import ConstraintSeq2SeqTrainingArguments, ConstraintSeq2SeqTrainer, gcn_model, DataCollatorForSeq2Seq_edges
...
def main():
    ...
    # !!! Sometimes default max_length is setting to 20.and set beams
    ...
    model_raw = AutoModelForSeq2SeqLM.from_pretrained(
        'local_bart',
    ...
    #模型加载（sometimes 从huggibg face连接出问题，使用本地文件）
    model = gcn_model(config, model_raw)
    model_path=model_args.model_name_or_path
    if(os.path.exists(model_path)):
        arg_path=model_path+r'/pytorch_model.bin'
        model.load_state_dict(torch.load(arg_path))
    ...
    def get_edges(text):
        ...
        for i, token1 in enumerate(doc):
            for j, token2 in enumerate(doc):
                if (token2 in list(token1.children)):
                    edges[0].append(i)
                    edges[1].append(j)
    ...
    def preprocess_function(examples):
        ...
        model_inputs["edges"]=edges  #加入边的关系
        return model_inputs
    ...
    if data_args.pad_to_max_length:
        ...
        data_collator = DataCollatorForSeq2Seq_edges(
            tokenizer,
            model=model,
